        ...
        <div class="box-header-menu-category">
            <div class="item-menu-category">
                @foreach ($data_collection as $item_coll)
                    <a href="{{ url('detail-collection', $item_coll->id) }}"
                        class="title-menu-category @if ($item_coll->id == $collection->id) menu-category-active @endif">{{ $item_coll->title }}</a>
                @endforeach
            </div>
        ...
        <div class="box-product-category box-product-category-four">
            @foreach ($collection_product as $product_item)
                <a href="{{ url('detail-product', $product_item->info->id) }}" class="item-product">
                    <img src="{{ asset('assets/images/Icon.png') }}" class="icon-cart-product" data-bs-toggle="offcanvas"
                        data-bs-target="#offcanvasRight" aria-controls="offcanvasRight"
                        onclick="addAttributeCart({{ $product_item->info->id }})">
                    @if ($product_item->interest == 1)
                        <img src="{{ asset('assets/images/heart-solid.svg') }}" class="icon-heart-product"
                            data-product-id="{{ $product_item->info->id }}">
                    @else
                    ...
                    <div class="box-info-sp">
                            ...
                            @php
                                if ($product_item->info->discount_type == 'percent') {
                                    $salePrice =
                                        ...
                                        ($product_item->info->price * $product_item->info->discount) / 100;
                                } else {
                        ...
                        @else
                            <div class="d-flex justify-content-between flex-wrap">
                                <div class="box-item-price">
                                    <p class="price-sale">{{ number_format($product_item->info->price, 0, '.', ',') }} đ
                                    </p>
                                </div>
                            </div>
                        @endif
