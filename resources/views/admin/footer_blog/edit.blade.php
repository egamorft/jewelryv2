...
@section('main')
    <main id="main" class="main d-flex flex-column justify-content-center">
        <div class="">
            <!-- Page Heading -->
            <h1 class="h3 mb-4 text-gray-800">Edit blog <strong>{{ $blog->title }}</strong></h1>
            <hr>
            <div class="container">
                <form method="POST" action="{{ route('admin.footer.blog.update', ['id' => $blog->id]) }}"
                        ...
                        <select class="form-select" id="categorySelector" name="category_id">
                            @foreach ($categories as $cate)
                                <option value="{{ $cate->id }}"
                                    {{ $cate->id == old('categories', $blog->category_id) ? 'selected' : '' }}>
                                    {{ $cate->name }}
                                </option>
                            @endforeach
                        </select>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="description" id="description" required class="ckeditor">{!! $blog->description !!}</textarea>
                    </div>
